...
#pragma region 8Bit Arithmetic
			...
			void ADD(const Register_unint8& reg) { ADD(reg.get()); }

			void ADC(uint8_t value);
			void ADC(const Register_unint8& reg) { ADC(reg.get()); }

			void SUB(uint8_t value) { registers.a.set(CP(value)); }
			void SUB(const Register_unint8& reg) { SUB(reg.get()); }

			void SBC(uint8_t value);
			void SBC(const Register_unint8& reg) { SBC(reg.get()); }

			void AND(uint8_t value);
			void AND(const Register_unint8& reg) { AND(reg.get()); }

			void XOR(uint8_t value);
			void XOR(const Register_unint8& reg) { XOR(reg.get()); }

			void OR(uint8_t value);
			void OR(const Register_unint8& reg) { OR(reg.get()); }

			uint8_t CP(uint8_t value);
			uint8_t CP(const Register_unint8& reg) { return CP(reg.get()); }
			void CPL();
#pragma endregion
...
#pragma region Bit Operations
			...
			void BIT(uint8_t bit, const Register_unint8& reg) { BIT(bit, reg.get()); }
